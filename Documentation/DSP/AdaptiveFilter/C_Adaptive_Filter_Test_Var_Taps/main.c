...
void main(int argc, char **argv){
    ...
    adaptive_tap_num = STARTING_TAP_TGT;
    ...
    double *adaptive_taps = malloc(MAX_TAP_TGT * sizeof(double));
    double *x = malloc(MAX_TAP_TGT * sizeof(double));
    ...
    double err_ave[ERR_AVE_LEN] = {1,1,1,1,1,1,1,1};

    double aved_err = 0;
    double old_aved_err = 0;
    double d_dt_err = 0;
    double d_dt_err_aved = 0;
    double d2_dt2_err = 0;
    double d2_dt2_err_aved = 0;
    ...
    if(ideal_taps && adaptive_taps && x && ideal_delay_line && error_plot && integral_error){
        ...
        for(uint32_t n = 0; n < MAX_TAP_TGT; ++n){
            adaptive_taps[n] = 0;
            x[n] = 0;
        }
        ...
        for(uint32_t n = 0; n < iterations; ++n){
            ...
            for(uint32_t m = 1; m < ERR_AVE_LEN; ++m) err_ave[m] = err_ave[m - 1];
            err_ave[0] = err * err;

            aved_err = 0;
            for(uint32_t m = 0; m < ERR_AVE_LEN; ++m){
                aved_err += err_ave[m] * (1.0 / (double)ERR_AVE_LEN);
            }

            d_dt_err_aved = d_dt_err;
            d_dt_err = aved_err - old_aved_err;
            old_aved_err = aved_err;

            d2_dt2_err_aved = d2_dt2_err;
            d2_dt2_err = d_dt_err - d_dt_err_aved;
        ...
        if(error_avg > LIMIT_FOR_ERROR){
            ...
            printf("Final Adaptive Taps: %u\nFinal Beta: %.10lf\n",adaptive_tap_num,beta);
